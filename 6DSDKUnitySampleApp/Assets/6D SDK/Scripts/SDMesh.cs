...
namespace SixDegrees
{
    public class SDMesh : MonoBehaviour
    {
        ...
        {
            return new Vector3Int(blockCoords.x / blocksPerChunk, blockCoords.y / blocksPerChunk, blockCoords.z / blocksPerChunk);
        }

        bool IsChunkTooFar(Vector3Int chunkCoords)
        {
            if (!destroyFarChunks || (Camera.main == null))
            {
                return false;
            }

            Vector3 camChunkCoords = GetChunkCoords(GetBlockCoords(Camera.main.transform.position));
