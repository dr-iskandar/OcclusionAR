...
public class ColorWheelControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //Set starting color to the 6D logo color
        ColorUtility.TryParseHtmlString("#004BD3", out Selection);
        PickColor(Selection);
    ...
    void Update()
    {
        ...
        else if (dragInner)
        {
            ...
            Vector2 dir = (transform.position - Input.mousePosition) / CanvasScale;
    ...
    public void OnClick()
    {
        ...
        if (Vector2.Distance(transform.position, Input.mousePosition) / CanvasScale <= halfSize &&
            Vector2.Distance(transform.position, Input.mousePosition) / CanvasScale >= halfSize - halfSize / 4)
        {
            dragOuter = true;
            return;
            //Check if click was in inner box
        }
        else if (Mathf.Abs(transform.position.x - Input.mousePosition.x) / CanvasScale <= halfSize / 2 &&
                 Mathf.Abs(transform.position.y - Input.mousePosition.y) / CanvasScale <= halfSize / 2)
        {
            dragInner = true;
            return;
        }
